...
fig, ax = plt.subplots(
    1, 3, sharex=True, sharey=True, figsize=(8.25, 3), constrained_layout=True
)
...
for i, treatment in enumerate(treatments):
    ...
    for j in range(len(bins) - 1):
        ...
        bin_treat_data = treat_data[
            (treat_data["prob_mort"] >= bins[j])
            & (treat_data["prob_mort"] < bins[j + 1])
        ]
        bin_non_treat_data = non_treat_data[
            (non_treat_data["prob_mort"] >= bins[j])
            & (non_treat_data["prob_mort"] < bins[j + 1])
        ]

        # Get the number of people in the bin with each outcome
        num_survived = len(bin_treat_data[bin_treat_data["mortality_in"] == 0]) + len(
            bin_non_treat_data[bin_non_treat_data["mortality_in"] == 0]
        )
        num_died = len(bin_treat_data[bin_treat_data["mortality_in"] == 1]) + len(
            bin_non_treat_data[bin_non_treat_data["mortality_in"] == 1]
        )
        ...
        if j == len(bins) - 2:
            ...
            ax[i].bar(
                j + 0.16, prob_died_treat, width=0.3, color=colors[1], label="Died"
            )
        else:
            # Plot the bar for the current bin and outcome
            ax[i].bar(j - 0.16, prob_survived_treat, width=0.3, color=colors[0])
            ax[i].bar(j + 0.16, prob_died_treat, width=0.3, color=colors[1])
...
# Save the figure
fig.savefig(f"results/sanity_check_all.tif", dpi=200, bbox_inches="tight")
