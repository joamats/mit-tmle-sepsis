...
rebuild_data <- function(sepsis_data, treatment) {
    ...
    # treatment should be "vent", "rrt", or "pressor"
    ...
    sepsis_data_new$charlson_comorbidity_index1 <- sepsis_data[, c("charlson_cont")]
    ...
    if (treatment == "vent") {
        ...
        sepsis_data_new$pressor1 <- sepsis_data[, c("pressor")]
        sepsis_data_new$vent1 <- sepsis_data[, c("vent")]
    ...
    sepsis_data_new$charlson_comorbidity_index2 <- sepsis_data[, c("charlson_cont")]
    ...
    if (treatment == "vent") {
        ...
        sepsis_data_new$pressor2 <- sepsis_data[, c("pressor")]
        sepsis_data_new$vent2 <- sepsis_data[, c("vent")]
    } else if (treatment == "pressor") {   
        sepsis_data_new$rrt2 <- sepsis_data[, c("rrt")]
        sepsis_data_new$vent2 <- sepsis_data[, c("vent")]
    }
