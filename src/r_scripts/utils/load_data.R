...
load_data <- function(cohort){
  ...
  if (file_path == "data/MIMIC_data.csv") {
    ...
    sepsis_data$los_hosp <- as.numeric(difftime(sepsis_data$dischtime, sepsis_data$admittime, units = 'days')) # Length of stay MIMIC
    sepsis_data$los_hosp[sepsis_data$los_hosp < 0] <- 0 # clean data to have minimum of 0 days
    ...
    sepsis_data$los_hosp <- (sepsis_data$hospitaldischargeoffset/1440) # Generate eICU Lenght of stay
  ...
  return(sepsis_data[, c("gender", "los_hosp", "ventilation_bin", "pressor", "rrt", "death_bin", "ethnicity_white",
