DROP TABLE IF EXISTS `db_name.icu_elos.pivoted_uo_24h`;
CREATE TABLE `db_name.icu_elos.pivoted_uo_24h` AS
with uo as
(
select
  patientunitstayid
  ...
  , case
    ...
    when lower(cellpath) like 'flowsheet|Flowsheet Cell Labels|I&O|Output (ml)|foley%'
...
AND (intakeoutputoffset > 0 AND intakeoutputoffset <= 1440) -- convert hours to minutes -> 60*24=1440
